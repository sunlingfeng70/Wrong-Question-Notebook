...
- **「记住我」登录选项**：登录表单新增「记住我」复选框（默认勾选）
  - 勾选时：auth Cookie 持久 30 天，浏览器重启后仍保持登录
  - 未勾选时：写入会话 Cookie，浏览器关闭即退出
  - 通过 `wqn-remember` 标记 Cookie 控制 Cookie 有效期，浏览器与服务端（含中间件会话刷新）保持一致
  - 邮箱验证/密码重置链接产生的会话默认为临时登录（关闭浏览器即退出）
